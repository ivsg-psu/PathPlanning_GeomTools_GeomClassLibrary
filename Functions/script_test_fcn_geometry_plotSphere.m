...
center = [1 3 5];
radius = [2]; %#ok<*NBRAK>
fcn_geometry_plotSphere(center,radius,[],fig_num);
...
centers = [1 3; 2 4];
radii = [2; 3];
fcn_geometry_plotSphere(centers,radii,[],fig_num);
...
centers = [1 2];
radii = 3;
fcn_geometry_plotSphere(centers,radii,'b',fig_num)
...
centers    = [1 2; 2 4; 3 5];
radii = [3; 4; 5];
fcn_geometry_plotSphere(centers,radii,'r.',fig_num)
...
% Do a light blue
fcn_geometry_plotSphere(centers,radii,[0.5 0.5 1],fig_num)
...
for i_circle=1:length(centers(:,1))
    fcn_geometry_plotSphere(centers(i_circle,:),radii(i_circle),[0.3*i_circle 0.3*i_circle 1],fig_num);
end
