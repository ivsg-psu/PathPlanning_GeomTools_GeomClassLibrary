function [flag_is_similar, points_XY_on_test_curve, minimum_distance_to_each_point, mean_error, max_error, std_dev_error] = ...
    fcn_geometry_compareCurves(reference_curve_type_string, reference_curve_parameters, test_curve_type_string, test_curve_parameters, varargin)
%% fcn_geometry_compareCurves
% Finds how "close" a test curve is to a reference curve.
%
% Given a reference curve type given by a string and curve parameters, and
% a test curve type given by a string and curve parameters, finds the
% minimum distance between each point in the testing set to the nearest
% point in the reference curve. If an optional threshold is given, returns
...
%
%      points_XY_on_test_curve: the XY locations on the test curve that were
%      tested.
%
...
% EXAMPLES:
%
% See the script: script_test_fcn_geometry_compareCurves
...
% Does user want to specify the curve_test_segment_length?
...
if flag_do_plots
    ...
    for ith_test_point = 1:NtestPoints
        ...
        plot(line_points(:,1),line_points(:,2),'.-','LineWidth',6,'Color',current_color,'MarkerSize',30);
    end
