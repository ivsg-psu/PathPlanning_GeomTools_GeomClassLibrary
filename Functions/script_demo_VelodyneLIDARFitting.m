...
time_iteration = 7; % 2, 4, 5, 6, 7*, 10, 23*, 32, 35*, 44
...
distance_center_to_center_threshold = 0.3;
...
mean_circle_center = mean(xyz_centers(indicies_of_rings_that_agree,:));
...
%
fcn_INTERNAL_plotResults(mean_circle_center, RingNames, currentScanRaw, rings_in_agreement, good_Regression_domains_to_keep, good_limits);

%% Find and plot all the points in each ring, in 3D
function fcn_INTERNAL_plotResults(mean_circle_center, RingNames, currentScanRaw, rings_in_agreement, good_Regression_domains_to_keep, good_limits)
figure(47464);
clf;
hold on;
grid on;
...
threshold_point_distance = 0.151;
...
for ith_ring = 1:N_rings
    ...
    % Find the close data
    distance_to_mean_center = sum((pointData3d(:,1:2)-mean_circle_center(1,1:2)).^2,2).^0.5;
