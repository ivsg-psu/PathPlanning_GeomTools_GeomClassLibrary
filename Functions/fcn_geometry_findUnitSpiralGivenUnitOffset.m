function [x_spiral,y_spiral] = fcn_geometry_findUnitSpiralGivenUnitOffset(offset, varargin)
...
function [unit_spiral_offsets,unit_spiral_end_angles_radians, unit_spiral_lengths, unit_spiral_end_XYs] = fcn_INTERNAL_calculateUnitSpiralData(fig_num)
...
for L0_index = 1:N_points
    if 0==mod(L0_index,5)
        fprintf(1,'Testing length %.0d of %.0d\n',L0_index,N_points);
    end
    L0 = unit_spiral_lengths(L0_index);

    if L0_index == N_points
        s  = 0:0.1:L0;
    else
        s  = L0;
    end
    ...
    unit_spiral_end_XYs(L0_index,:) = [x_arc(end) y_arc(end)];
    ...
    if 1==0
        ...
        unit_spiral_end_angles_radians(L0_index,:) = atan2(unit_tangent(2),unit_tangent(1));
    else
        unit_spiral_end_angles_radians(L0_index,:) = L0/2;
        unit_tangent = [cos(unit_spiral_end_angles_radians(L0_index,:)) sin(unit_spiral_end_angles_radians(L0_index,:))];
    end

    unit_orthogonal = unit_tangent*[0 1; -1 0];
    circle_center = (1/Kf)*unit_orthogonal + [x_arc(end) y_arc(end)];

    unit_spiral_offsets(L0_index,:)            = circle_center(1,2) - (1/Kf);


    % Plot the circle
    if 0==mod(L0_index,100) && ~isempty(fig_num)
        figure(fig_num);
        subplot(2,2,1);
...
% Show that final angle is exactly half the length of euler spiral
ratio_error = 2*unit_spiral_end_angles_radians./unit_spiral_lengths - ones(length(unit_spiral_lengths(:,1)),1);
assert(max(abs(ratio_error))<1E-7);
