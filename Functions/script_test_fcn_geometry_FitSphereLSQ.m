% script_test_fcn_geometry_FitSphereLSQ.m
% This is a script to exercise the function: fcn_geometry_FitSphereLSQ.m
...
%% Use the rawdata.mat in the Data folder to start, the loaded data is a struct containing two fields
% rawdata.GPS_SparkFun_Temp_ENU and rawdata.Lidar_pointcloud_cell
data_folder = fullfile("..","Data");
addpath(data_folder)
load('rawdata_lidar.mat')
%% Grab the GPS and Lidar Data
GPS_SparkFun_Temp_ENU = rawdata.GPS_SparkFun_Temp_ENU;
Lidar_pointcloud_cell = rawdata.Lidar_pointcloud_cell;
...
for idx_fit = fit_range
    ...
    XYZ_array = current_scan(:,1:3);
    Intensity = current_scan(:,4);
    [C_sphere,R_sphere,E_total] = fcn_LidarPoseEstimation_FitSphereLSQ(XYZ_array);
...
for idx_scan = 1:length(Lidar_pointcloud_cell)
    current_scan_cell = Lidar_pointcloud_cell{i_scan};
    XYZ_array = current_scan_cell(:,1:3);
    Intensity_array = current_scan_cell(:,4);
    ptCloud_real = pointCloud(XYZ_array,'Intensity',Intensity_array);
    [model,inlierIndices] = pcfitsphere(ptCloud_real,maxDistance);
%     globe = select(model, inlierIndices);
    XYZ_array_globe = XYZ_array(inlierIndices,:);
    Intensity_array_globe = Intensity_array(inlierIndices,:);
    ptCloud_globe = pointCloud(XYZ_array_globe,'Intensity',Intensity_array_globe);
    if ~isnan(ptCloud_real.Location)
        view(LidarViewer,ptCloud_globe)
        LidarViewer.Axes.Title.String = sprintf("Frame number: %d", idx_fit);
        pause(pause_time)
    end
