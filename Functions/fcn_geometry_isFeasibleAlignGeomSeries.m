function [joint_feasibility, joint_distance_of_failure] = ...
    fcn_geometry_isFeasibleAlignGeomSeries(input_types, input_parameters, continuity_level, varargin)
%% fcn_geometry_isFeasibleAlignGeomSeries
% Given a sequence of N geometries that are in series with each other, and a given type of continuity, checks if it is possible to achieve
% levels of continuity between fits. For each of the N-1 joints of the N
% geometries, returns 1 if the geometries can be joined or 0 if they
% cannot. As well, returns the positive or negative values of closeness to
% violation of agreement, with the more positive numbers meaning higher
% agreements, 0 being the threshold, and negative numbers meaning ever
% higher violations of agreement.
%
% The levels of continuity include:
%
...
%  intersect at at least 1 point. Th
%
...
%
%      continuity: the level of continuity being sought between adjacent
%      geometries
%
%      (OPTIONAL INPUTS)
%
...
% EXAMPLES:
%
% See the script: script_test_fcn_geometry_isFeasibleAlignGeomSeries
...
NfitsInSequence = length(input_types);

% Fill in starter values for the outputs
joint_feasibility         = zeros(NfitsInSequence-1,1);
joint_distance_of_failure = nan(NfitsInSequence-1,1);


% Loop through fits, connecting them together
for ith_fit = 1:NfitsInSequence-1
    current_fit_type = fcn_INTERNAL_covertComplexShapeNamesToSimpleNames(input_types{ith_fit});
    next_fit_type    = fcn_INTERNAL_covertComplexShapeNamesToSimpleNames(input_types{ith_fit+1});

    current_fit_parameters = input_parameters{ith_fit};
    next_fit_parameters    = input_parameters{ith_fit+1};


    % Call function
    [flag_is_feasible, feasibility_distance] = ...
        fcn_geometry_isFeasibleAlignGeomPair(current_fit_type, current_fit_parameters, next_fit_type, next_fit_parameters, continuity_level, (threshold), (-1));

    joint_feasibility(ith_fit,1) = flag_is_feasible;
    joint_distance_of_failure(ith_fit,1) = feasibility_distance;
